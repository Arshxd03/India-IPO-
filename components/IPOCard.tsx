...
import { 
  ...
  ChevronDown, ChevronUp, Loader2, Info, Search, ShieldCheck
} from 'lucide-react';
...
const IPOCard: React.FC<IPOCardProps> = ({ ipo, onCheckInvestment, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <div 
      ...
      className={`bg-white dark:bg-[#0f172a] p-8 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 flex flex-col h-full shadow-lg hover:shadow-2xl transition-all duration-500 group relative overflow-hidden ${isListed ? 'cursor-pointer' : ''} ${isExpanded ? 'ring-2 ring-indigo-500/20' : ''}`}
    >
      <div className="flex justify-between items-start mb-6">
        <div className="space-y-3 flex-1">
          ...
          <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-tight group-hover:text-emerald-500 transition-colors">
            {ipo.name}
          </h3>
        </div>
      ...
      {!isListed ? (
        <>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="p-4 bg-emerald-500/5 dark:bg-emerald-500/10 rounded-2xl border border-emerald-500/20 dark:border-emerald-500/30 flex items-center gap-3 group/stat">
              <Zap size={20} className="text-emerald-500 group-hover/stat:scale-110 transition-transform" strokeWidth={2.5} />
              <div className="overflow-hidden">
                <p className="text-[9px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Expected GMP</p>
                <p className="text-xl font-black text-slate-900 dark:text-white truncate">₹{ipo.gmp}</p>
              </div>
            </div>
            <div className="p-4 bg-indigo-500/5 dark:bg-indigo-500/10 rounded-2xl border border-indigo-500/20 dark:border-indigo-500/30 flex items-center gap-3 group/stat">
              <Users2 size={20} className="text-indigo-500 group-hover/stat:scale-110 transition-transform" strokeWidth={2.5} />
              <div className="overflow-hidden">
                <p className="text-[9px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">Subscription</p>
                <p className="text-xl font-black text-slate-900 dark:text-white truncate">{ipo.subscription}</p>
              </div>
            </div>
          </div>
          <div className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-800/50 mb-8">
            <div className="flex justify-between items-center gap-4">
              <div className="overflow-hidden">
                ...
                <p className="text-lg font-black text-slate-900 dark:text-white truncate">{ipo.lotSize} Shares</p>
              </div>
        ...
        <div className="space-y-4 mb-8">
           <div className="flex items-center gap-2 mb-4">
              <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-[0.2em] ${
                ...
                {ipo.returns! > 30 ? 'Multi-Bagger' : ipo.returns! > 0 ? 'Positive Listing' : 'Sub-Issue List'}
              </span>
              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-auto flex items-center gap-1">
                {isExpanded ? 'Minimize details' : 'View AI Analysis'}
              </span>
           </div>
           <div className="p-6 bg-emerald-600 dark:bg-emerald-500 text-white rounded-[2.5rem] shadow-xl relative overflow-hidden group-hover:scale-[1.02] transition-transform">
             <ArrowUpRight size={80} className="absolute right-[-10%] top-[-20%] opacity-10" />
             <div className="flex justify-between items-end relative z-10">
               <div>
                 <p className="text-[9px] font-black uppercase tracking-widest opacity-80 mb-1">Current Price</p>
                 <p className="text-3xl font-black tracking-tight">₹{ipo.currentPrice?.toLocaleString('en-IN')}</p>
               </div>
               <div className="text-right">
                 <p className="text-[9px] font-black uppercase tracking-widest opacity-80 mb-1">Total Return</p>
                ...
                {isDeepLoading ? (
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="space-y-2">
                      <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Business Overview</p>
                      <p className="text-sm font-medium leading-relaxed text-slate-600 dark:text-slate-300">
                        {detailedAnalysis?.split('\n')[0] || detailedAnalysis}
                      </p>
                    </div>
                    <div className="space-y-3">
                      <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Market Insight</p>
      ...
      <div className="mt-auto flex flex-col gap-3">
        <div className="flex gap-3">
          {ipo.status === 'Open' ? (
            <button onClick={handleCheck} className="flex-1 py-4 bg-emerald-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">
              Invest Now
            </button>
          ) : isListed ? (
            <a onClick={e => e.stopPropagation()} href="https://linkintime.co.in/initial_offer/public-issues.html" target="_blank" className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20 hover:bg-indigo-700 transition-all">
              Check Allotment <ShieldCheck size={12} />
            </a>
          ) : (
